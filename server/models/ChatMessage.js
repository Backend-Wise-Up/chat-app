...
chatMessageSchema.statics.createReplyInChatRoom = async function (messageId, message, postedByUser) {
  try {
    const post = await this.create({
      messageId,
      message,
    ...
    const aggregate = await this.aggregate([
      ...
      { $unwind: '$messageInfo' },
      { $unwind: '$messageInfo.userIds' },
      // do a join on another table called users, and 
      // get me a user whose _id = userIds
      {
        $lookup: {
          from: 'users',
          localField: 'messageInfo.userIds',
          foreignField: '_id',
          as: 'messageInfo.userProfile',
        }
      },
      { $unwind: '$messageInfo.userProfile' },
      // group data
      {
        $group: {
          _id: '$messageInfo._id',
          postId: { $last: '$_id' },
          messageId: { $last: '$messageInfo._id' },
          message: { $last: '$message' },
          type: { $last: '$type' },
          postedByUser: { $last: '$postedByUser' },
          readByRecipients: { $last: '$readByRecipients' },
          messageInfo: { $addToSet: '$messageInfo.userProfile' },
          createdAt: { $last: '$createdAt' },
          updatedAt: { $last: '$updatedAt' },
        }
      }
